...
```{r}
pacman::p_load(ggiraph, plotly, patchwork, DT, tidyverse,knitr) 
```
...
participants$joviality <- round(participants$joviality, 2)
...
```{r}
#create new dataset 
grouped_data <- financial_journal %>%
  
#recode ID from dbl to chr, year_mth
    mutate(participantId = as.character(participantId),
      year = as.numeric(format(as.Date(paste(financial_journal$timestamp, "-01", sep="")), "%Y")),
         month = as.numeric(format(as.Date(financial_journal$timestamp), "%m")),
         year_mth = format(as.Date(financial_journal$timestamp), "%Y-%m"),
