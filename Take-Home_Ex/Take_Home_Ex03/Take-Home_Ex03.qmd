...
From the table above, we noticed that there are word that might not be useful for our analysis. For e.g. `and` in row 6. Therefore, we would like to exclude such word in our analysis. From the code chunk below, `stop_words()` from the `[tidytext]` package is used to aid us in the filtering progress while `anti_join()` from the `[dplyr]` package is used to remove all stop_words from the analysis.
...
Notably, the word above remain unidentified as they might be too generic or they could potentially belong to more than one category. For instance, Frozen could be applicable to Fishing, or to other type of Food. Thus, it is better to remain unidentified as the keyword could be picked up in the
...
-   The distribution by countries are spread among the countries, with higher concentration in ZH, Oceanus and Marebak. This is in line with the distribution in section 5.
