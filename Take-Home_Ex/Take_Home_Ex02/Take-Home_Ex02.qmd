...
In this take-home exercise, temporal patterns for individual entries and between entities are identified using the knowledge graph FishEye created from trade records. It categorizes the type of business relationship patterns. In addition, we evaluate the sets of predicted knowledge graph links (Bundles) to determine which set is more reliable in completing the graph.
...
Among the 17 hscode identified in the treemap, we noticed that there are two codes that belongs to Chapter 30. In terms of volume, `306170` and `304620` are ranked first and sixth respectively. Therefore, we would like to match with the set of predicted knowledge graph links from FishEye to determine if we would get any insight.
