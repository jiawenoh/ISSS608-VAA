...
In this take-home exercise, temporal patterns for individual entries and between entities are identified using the knowledge graph FishEye created from trade records. It categorizes the type of business relationship patterns and possibly, identify companies that fit a pattern of illegal fishing.
...
## 3.6 Creating a Master ID data frame
...
A `treemap` displays hierarchical data as a set of nested rectangles, where each group is represented by an area that is proportional to its value. We would like to examine the distribution by **hscode** and **year.**
...
-   HS code has a hierarchical model. It is composed fo six digits and could be broken down into chapter/heading/subheading with two digits each. As observed, hscode that starts with `30` occupies a higher weightage as well. From the graph above, hscode `306170` and `304620` are rank top and sixth position.

### 5.1 Network graph for Hscode (306170)
...
#plot network graph 
visNetwork(MC2_306170_nodes, MC2_306170_edges, main ="Trade flow for
           <br>hscode 361070 <br>") %>%
  visIgraphLayout(layout = "layout_with_fr") %>%
    visOptions(highlightNearest = TRUE,
             nodesIdSelection = TRUE) %>%
...
Eigenvector centrality measures the importance or influence of a node based on its connections to other highly central nodes. A high eigenvector centrality indicates an important/influential connection to other nodes. The blue nodes above could represents key players or market leader in the network.

###        5.2 Network graph for Hscode and year
...
###   5.3 Identifying hscode

Using the set of predicted knowledge graph links from FishEye, we would like to examine which types it belongs to in the marine and fishing industries.
...
```{r}
d3tree(hscount30, rootname = "Distribution of fish types")
```

**Observations:**

-   From the predicted knowledge graph links, Cod2 have the highest amount of transactions in chapter 30. *(hscode that begins with '30')*
...
#### 5.3.1 Incomplete hscode 

Extract source and target organizations who have incomplete hscode.

```{r}
#| code-fold: true

#filter for organization with 5 digits hscode 
...
kable(head(combined_nodes_filtered), "simple")
```
...
visNetwork(combined_nodes_filtered_addid, combined_edges_cleaned_newedges_test, main ="Top 30 source/targets 
